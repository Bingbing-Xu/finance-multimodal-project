...
class Twitter(object):
    ...
    def load_dataset(self):
        if self.config.dataset_name == "Twitter15":
            ...
            train = json.load(open(os.path.join(self.root_dir, 'tw15_train.json'), encoding='utf-8'))
            dev = json.load(open(os.path.join(self.root_dir, 'tw15_dev.json'), encoding='utf-8'))
            test = json.load(open(os.path.join(self.root_dir, 'tw15_test.json'), encoding='utf-8'))
        else:
            train = json.load(open(os.path.join(self.root_dir, 'tw17_train.json'), encoding='utf-8'))
            dev = json.load(open(os.path.join(self.root_dir, 'tw17_dev.json'), encoding='utf-8'))
            test = json.load(open(os.path.join(self.root_dir, 'tw17_test.json'), encoding='utf-8'))
...
"""class StockKnow(object):
    def __init__(self, config, root_dir, tokenizer):
        ...
        # 添加消融实验配置支持
        self.use_text = getattr(config, 'use_text', True)  # 默认使用文本
        self.use_description = getattr(config, 'use_description', True)  # 默认使用描述
        self.use_reason = getattr(config, 'use_reason', True)  # 默认使用推理
    ...
    def load_dataset(self):
        if self.config.task == "Stock":
            train = json.load(open(os.path.join(self.root_dir, 'train.json'), encoding='utf-8'))
            dev = json.load(open(os.path.join(self.root_dir, 'dev.json'), encoding='utf-8'))
            test = json.load(open(os.path.join(self.root_dir, 'test.json'), encoding='utf-8'))
        elif self.config.task == "TW17K":
            train = json.load(open(os.path.join(self.root_dir, 'tw17_train.json')))
            dev = json.load(open(os.path.join(self.root_dir, 'tw17_dev.json')))
            test = json.load(open(os.path.join(self.root_dir, 'tw17_test.json')))
        elif self.config.task == "TW15K":
            train = json.load(open(os.path.join(self.root_dir, 'tw15_train.json')))
            dev = json.load(open(os.path.join(self.root_dir, 'tw15_dev.json')))
            test = json.load(open(os.path.join(self.root_dir, 'tw15_test.json')))
...
class StockKnow(object):
    def __init__(self, config, root_dir, tokenizer):
        ...
        self.use_reason = getattr(config, 'use_reason', True)

    # ---------- CLS 处理（用于 Text-Only 变体）----------
    def bert_cls_process(self, data):
        ids, texts, describs, reasons, imgs, labels, targets = data
        processed_data = []
        for i in range(len(ids)):
            instance = {}
            # 根据控制参数组合输入
            ...
            if self.use_description:
                describ = ' '.join(describs[i].strip().split()[:self.config.reserve_length])
                parts.append(describ)
            # CLS 不使用 reason
    ...
    # ---------- MLM 处理（用于 Caption-Only 和 Text+Caption）----------
    def bert_mlm_process(self, data):
        ids, texts, describs, reasons, imgs, labels, targets = data
        processed_data = []
        for i in range(len(ids)):
            instance = {}
            # 构建 content（用于填充模板的文本部分）
            content_parts = []
            if self.use_text:
                content_parts.append(texts[i])
            if self.use_description:
                describ = ' '.join(describs[i].strip().split()[:self.config.reserve_length])
                content_parts.append(describ)
            content = self.tokenizer.sep_token.join(content_parts) if content_parts else ''
    ...
    def bert_mlmp_process(self, data):
        ...
        for i in range(len(ids)):
            ...
            # 针对 RoBERTa 的 token_type_ids 修正（可选）
            if self.config.encoder_type == 'roberta':
                x_ids, y_ids = torch.where(reason_feature['input_ids'] == self.tokenizer.eos_token_id)
                if len(y_ids) >= 2:
                    reason_feature['token_type_ids'][0][y_ids[-2]: y_ids[-1]] = 1
    ...
    def load_dataset(self):
        ...
        def get_data(data):
            ...
            for item in data:
                ...
                ids.append(str(item['ImageID']))
                images.append(str(item['ImageID']))
                labels.append(int(item['gt_label']) + 1)   # 假设标签为 -1,0,1 → 转为 0,1,2
                text = item['text'].replace('$T$', 'Jake Paul')  # 替换占位符
                texts.append(normalize_word(text))
                descrips.append(normalize_word(item['description']))
                reasons.append(normalize_word(item['reason']))
            return ids, texts, descrips, reasons, images, labels, targets
        ...
        if self.config.few_shot is not None:
            train_data = self.fewshot_dataset(train_data)
        print('The number of training samples is : {}'.format(len(train_data[0])))
